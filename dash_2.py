    ...
    if True :
        ...
        # Capturing video
            ...
            # Passing individual frames of video
                canvas = funct(frame)[0]
            # Showing frame returned from classify function
                cv2.imshow('Video', canvas)
                st.write(funct(frame)[1])
                L.append(funct(frame)[1])
...
with col3:
    ...
    while run and stream.is_active(): 
        ...
        if db>20 and db<40:
            ...
            st.write(db,'Very loud, Consider using hearing protection')
        elif db>=110 and db<120:
            st.write(db,'Uncomfortable,Use hearing protection')
        elif db>=120:
            st.write(db,'Painful & Dangerous,Use hearing protection or AVOID!')
        A.append(db)
